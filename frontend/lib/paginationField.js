  ...
  return {
    ...
    read(existing = [], { args, cache }) {
      ...
      if (items.length !== first) {
        // we dont have any items, we must go to the newtwork to fetch them
        return false;
      }

      // if there are items, just return them from the cache, and we dont need to goto network

      if (items.length) {
        console.log(`There are ${items.length} in cache`);
        return items;
      }
